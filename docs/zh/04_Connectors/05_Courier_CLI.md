# Courier CLI 设计

**Version**: 1.1.0
**Status**: Draft
**Related**: FEAT-0191

---

## 1. 概述

Courier CLI 是 Courier 服务生命周期管理的唯一接口。它仅负责服务的**启动、停止和监控**，不负责消息处理。所有消息操作通过 `mailbox` 命令完成。

### 1.1 设计原则

1. **单一职责**: Courier CLI 只管理服务进程
2. **与数据分离**: 消息状态管理是 Courier 服务的内部职责，CLI 不直接干预
3. **简单直接**: 命令语义清晰，行为可预测
4. **快速失败**: 操作失败立即返回错误，不阻塞

---

## 2. 命令概览

```
monoco courier
├── start         # 启动 Courier 服务
├── stop          # 停止 Courier 服务（优雅关闭）
├── restart       # 重启 Courier 服务
├── kill          # 强制停止 Courier 服务（不优雅）
├── status        # 查看服务状态
└── logs          # 查看服务日志
```

**注意**: Courier CLI **没有** `send`、`archive`、`config` 等命令。这些功能通过 `mailbox` 命令或 Courier 服务自动完成。

---

## 3. 服务管理命令

### 3.1 `courier start`

启动 Courier 守护进程。

```bash
# 基本用法
monoco courier start                          # 后台启动服务
monoco courier start --foreground             # 前台启动（调试用）
monoco courier start --debug                  # 调试模式（详细日志）

# 配置选项
monoco courier start --config /path/to/config.yaml  # 指定配置文件
```

**行为**:
- 创建 PID 文件 `.monoco/run/courier.pid`
- 启动日志写入 `.monoco/log/courier.log`
- 启动 HTTP API 服务（默认端口 8080）
- 初始化所有启用的适配器
- 开始监听 Webhook 和轮询

**返回码**:
- `0`: 启动成功
- `1`: 启动失败
- `2`: 服务已在运行

---

### 3.2 `courier stop`

优雅地停止 Courier 服务。

```bash
# 基本用法
monoco courier stop                           # 发送停止信号
monoco courier stop --timeout 30              # 等待30秒后强制停止

# 选项
monoco courier stop --wait                    # 阻塞直到服务停止
```

**行为**:
1. 发送 SIGTERM 信号给 Courier 进程
2. Courier 开始优雅关闭：
   - 停止接收新消息（Webhook 返回 503）
   - 完成进行中的发送任务
   - 等待已认领消息超时或完成
   - 关闭适配器连接
   - 清理临时文件
3. 如果在 timeout 内未完成，发送 SIGKILL

**注意**: 优雅停止可能较慢，但不会丢失消息。

---

### 3.3 `courier restart`

重启 Courier 服务。

```bash
# 基本用法
monoco courier restart                        # 停止后重新启动
monoco courier restart --force                # 如果停止失败，强制重启
```

**行为**:
- 等同于 `stop` + `start`
- 保持配置不变

---

### 3.4 `courier kill`

强制停止 Courier 服务。

```bash
# 基本用法
monoco courier kill                           # 立即强制停止
monoco courier kill --signal INT              # 发送特定信号（默认 SIGKILL）
```

⚠️ **警告**:
- 立即终止进程，**不执行清理逻辑**
- 可能导致：
  - 内存中的锁状态丢失
  - 已认领消息状态不确定
  - 发送中的消息状态不确定
  - 临时文件残留
  - 外部平台连接异常

**使用场景**:
- 服务无响应，正常 stop 无效
- 需要立即释放资源
- 紧急情况下快速恢复

**不建议使用场景**:
- 正常维护（请用 `stop`）
- 可以等待的任务完成

---

### 3.5 `courier status`

查看 Courier 服务状态。

```bash
# 基本用法
monoco courier status                         # 显示服务状态
monoco courier status --json                  # JSON 格式输出
monoco courier status --watch                 # 持续监控
```

**输出示例**:

```
┌─────────────────────────────────────────────────────────────────┐
│ Courier Service Status                                           │
├─────────────────────────────────────────────────────────────────┤
│ Service:     running                                             │
│ PID:         12345                                               │
│ Uptime:      2 hours 15 minutes                                  │
│ Version:     1.0.0                                               │
│ API:         http://localhost:8080                               │
│                                                                  │
│ Adapters:                                                        │
│   lark       🟢 connected     (webhook: 8080)                   │
│   email      🟢 connected     (poll: 30s)                       │
│   slack      ⚪ disabled                                         │
│                                                                  │
│ Metrics:                                                         │
│   Messages Received:  152                                        │
│   Messages Sent:      43                                         │
│   Messages Claimed:   2                                          │
│   Messages Pending:   5                                          │
│   Errors:             0                                          │
└─────────────────────────────────────────────────────────────────┘
```

**状态说明**:
- 🟢 `connected`: 正常连接
- 🟡 `connecting`: 正在连接
- 🔴 `error`: 连接错误
- ⚪ `disabled`: 已禁用

---

### 3.6 `courier logs`

查看 Courier 服务日志。

```bash
# 基本用法
monoco courier logs                           # 显示最近100行
monoco courier logs --follow                  # 实时跟踪
monoco courier logs --lines 500               # 显示最近500行

# 过滤
monoco courier logs --level error             # 仅显示错误
monoco courier logs --adapter lark            # 仅显示 lark 适配器日志
monoco courier logs --since "1h"              # 最近1小时
```

---

## 4. kill vs stop 对比

| 特性 | `stop` | `kill` |
|------|--------|--------|
| 信号 | SIGTERM | SIGKILL |
| 优雅关闭 | ✅ 是 | ❌ 否 |
| 等待任务完成 | ✅ 是 | ❌ 否 |
| 清理资源 | ✅ 是 | ❌ 否 |
| 锁状态保存 | ✅ 是 | ❌ 丢失 |
| 数据丢失风险 | 低 | 高 |
| 适用场景 | 正常维护 | 紧急恢复 |

---

## 5. 典型工作流

### 5.1 日常启动流程

```bash
# 1. 检查状态
monoco courier status

# 2. 如果未运行，启动服务
monoco courier start

# 3. 确认启动成功
monoco courier status
```

### 5.2 故障恢复流程

```bash
# 1. 检查状态
monoco courier status
# Output: Service: error (adapter lark: connection timeout)

# 2. 查看错误日志
monoco courier logs --level error --lines 50

# 3. 尝试优雅重启
monoco courier restart

# 4. 如果重启失败，强制停止后启动
monoco courier kill
monoco courier start
```

### 5.3 配置更新流程

```bash
# 1. 停止服务
monoco courier stop

# 2. 编辑配置文件
vim .monoco/config/courier.yaml

# 3. 启动服务
monoco courier start
```

---

## 6. 错误处理

| 错误场景 | 命令 | 返回码 | 提示信息 |
|----------|------|--------|----------|
| 服务已在运行 | `start` | 2 | `Error: Courier is already running (PID: 12345)` |
| 服务未运行 | `stop`/`kill`/`restart` | 1 | `Error: Courier is not running` |
| PID 文件存在但进程不存在 | `start` | 1 | `Error: Stale PID file found, cleaning up...` |
| 权限不足 | `start` | 3 | `Error: Permission denied to write PID file` |
| 端口被占用 | `start` | 4 | `Error: Port 8080 is already in use` |

---

## 7. 与 Mailbox CLI 的关系

```
┌─────────────────────────────────────────────────────────────┐
│  Agent 用户                                                  │
│                                                             │
│  ┌──────────────┐      ┌──────────────┐                    │
│  │    mailbox   │      │   courier    │                    │
│  │    list      │      │   start      │                    │
│  │    read      │      │   stop       │  ◄── 仅服务管理     │
│  │    send      │      │   status     │                    │
│  │    claim ────┼──────┼──────────────┤                    │
│  │    done   ───┤      │              │                    │
│  │    fail   ───┘      │              │                    │
│  └──────────────┘      └──────────────┘                    │
│         │                      │                            │
│         ▼                      ▼                            │
│  ┌─────────────────────────────────────────────────────┐   │
│  │              Courier Service (Daemon)                │   │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐          │   │
│  │  │ Lock Mgr │  │ Archive  │  │  Retry   │          │   │
│  │  │  (claim) │  │  (done)  │  │  (fail)  │          │   │
│  │  └──────────┘  └──────────┘  └──────────┘          │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```

---

## 相关文档

- [01_Architecture](01_Architecture.md) - 整体架构设计
- [02_Mailbox_Protocol](02_Mailbox_Protocol.md) - 消息协议 Schema 规范
- [03_Mailbox_CLI](03_Mailbox_CLI.md) - Mailbox CLI 命令设计
- [04_Courier_Service](04_Courier_Service.md) - Courier 服务架构设计

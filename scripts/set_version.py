...
def update_version(file_path, new_version):
    ...
    if path.name == "pyproject.toml":
        ...
        content = re.sub(pattern, f'\\g<1>{new_version}\\g<2>', content, flags=re.MULTILINE)
    ...
    if content != original_content:
        ...
        print(f"⚠️  No changes or match found in {path.relative_to(Path(__file__).parent.parent)}")
        return False

...
export default function DetailedView() {
    ...
    const getStatusIntent = (status: string) => {
        switch (status.toLowerCase()) {
            case 'open': return Intent.NONE;
            case 'in_progress': return Intent.PRIMARY;
            case 'done': return Intent.SUCCESS;
            case 'closed': return Intent.DANGER;
    ...
    const getTypeIcon = (type: string) => {
        switch (type.toLowerCase()) {
            ...
            case 'chore': return 'clean';
            case 'docs': return 'document';
            case 'refactor': return 'git-merge';
            case 'epic': return 'crown'; // Icon for Epic
            case 'group': return 'folder-close';
            default: return 'help';
        }
    };
    ...
    const renderRow = (issue: any, level: number = 0) => {
        ...
        return (
            <React.Fragment key={issue.id}>
                <tr 
                    className={`hover:bg-surface-highlight cursor-pointer transition-colors ${level === 0 ? 'bg-surface-subtle/30' : ''}`}
                    onClick={() => setSelectedIssueId(issue.id)}
                >
                    <td style={{ paddingLeft }} className="font-mono text-xs text-text-muted whitespace-nowrap">
                        <div className="flex items-center gap-2">
                            ...
                            {!hasChildren && level > 0 && <div className="w-[20px]" />} {/* Spacer */}
                            {issue.id}
                        </div>
                    </td>
                    <td>
                        <div className="flex items-center gap-2">
                            <Icon icon={getTypeIcon(issue.type) as any} size={14} className="text-text-muted" />
                            <span className="uppercase text-xs font-semibold text-text-secondary">{issue.type}</span>
                        </div>
                    </td>
                    ...
                    <td>
                        <Tag intent={getStatusIntent(issue.status)} minimal round>{issue.status}</Tag>
                    </td>
                ...
                <div className="flex justify-between items-end mb-4">
                    <div>
                        ...
                        <p className="text-sm text-text-muted">
                            {issues.length} issues across the project.
                        </p>
                    </div>

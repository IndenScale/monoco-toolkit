...
type: feature
status: closed
title: Support Hierarchical Issue Organization
...
1. **Discovery**: `monoco issue scope/lint` 默认仅扫描一级目录（Shallow）。必须支持 `-r/--recursive` 参数以开启递归扫描（Deep）。
2. **Linting**: `lint` 规则需放宽，允许 Issue 存在于子目录。如果不开启 `-r`，则忽略子目录违规（因为看不到）。
3. **Creation**: `monoco issue create` 支持 `--subdir` 参数。
4. **Structure Integrity**: 当 Issue 状态流转（如 `open` -> `closed`）时，**必须保留**其原有的子目录结构，禁止将其扁平化移动到根目录。

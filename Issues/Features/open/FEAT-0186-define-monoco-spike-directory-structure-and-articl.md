...
updated_at: '2026-02-06T05:02:53'
parent: EPIC-0000
dependencies: []
related: []
domains: []
...
- [ ] 定义 `.references/` 层级目录结构规范
- [ ] 定义 `repos/` 子目录结构和命名约定
- [ ] 定义 `articles/` 子目录结构和命名约定
- [ ] 设计 article front matter 模板（YAML 字段规范）
- [ ] `monoco init` 命令注入模板到 `.references/articles/template.md`
- [ ] 更新 CLAUDE.md 文档中的 Spike 章节
...
- [ ] 命名规则：使用仓库名称小写，kebab-case
- [ ] 每个 repo 为完整 git clone（保留 .git 历史）
- [ ] 示例：`repos/kimi-cli/`, `repos/claude-code/`

### 3. articles/ 规范

- [ ] 二级目录按来源组织（小写）：`articles/openai/`, `articles/anthropic/`
- [ ] 文章文件使用 kebab-case：`introducing-frontier.md`
- [ ] i18n 支持：`articles/openai/zh/` 子目录存放翻译
- [ ] 所有文章必须包含 front matter

### 4. Front Matter 模板设计

- [ ] 模板字段默认值使用 `UNKNOWN`，允许不确定时保留占位
- [ ] 必需字段：`id`, `title`, `source`, `date`, `type`
- [ ] 可选字段：`author`, `tags`, `summary`, `related`
- [ ] i18n 字段：`language`, `translations`
- [ ] 治理字段：`domain`, `company`

### 5. monoco init 集成

- [ ] 修改 `monoco init` 命令
- [ ] 创建 `.references/articles/` 目录（如果不存在）
- [ ] 注入 `template.md` 到 `.references/articles/template.md`

### 6. 文档更新

- [ ] 更新 CLAUDE.md Spike 章节，添加目录结构说明
- [ ] 添加 front matter 字段说明

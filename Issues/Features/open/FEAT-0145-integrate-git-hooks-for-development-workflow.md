...
updated_at: '2026-02-02T11:45:00'
parent: EPIC-0030
dependencies: []
related:
- FEAT-0141
...
将 Git Hooks 集成到 Monoco 工作流中，确保开发规范在提交阶段被强制执行。当前缺少必要字段（如关闭 Issue 的 `solution` 字段）的问题可能破坏索引器或依赖关系，手动修复容易出错。通过 `pre-commit` 钩子运行 `monoco issue lint` 可以在提交前拦截不合规的更改。

## Objective

Integrate Git Hooks (pre-commit) into the Monoco workflow to ensure data integrity and process compliance.
...
- [ ] 实现 staged Issue 文件检测（只检查变更的 Issue）
...
- **pre-push hook**：使用场景不明确，后续按需添加
- **post-checkout hook**：自动同步 Issue 状态的必要性待验证
...
- 移除 `pre-push` 和 `post-checkout` 任务，聚焦核心需求

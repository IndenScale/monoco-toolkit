...
- [ ] `monoco issue close` 默认执行 `--prune` 操作（删除分支/Worktree），除非显式指定 `--no-prune`。
- [ ] `monoco issue close` 支持（或建议）自动化合并流程，优先尝试智能合并。
- [ ] 完成对 `touched files` (Issue `files` 字段) 追踪机制的深度调查报告，评估其作为“智能合并”依据的可行性。
- [ ] 更新 `monoco/features/issue/resources/zh/AGENTS.md` 和相关 Skill 文档，明确合并规范和 Fallback 策略。

## 技术任务

### Phase 1: 机制增强 (Implementation)
- [ ] 修改 `monoco issue close` 命令参数，将 `prune` 默认设为 `True`。
- [ ] 增强 `monoco issue close` 的交互提示，在删除分支前给出明确的最终状态确认。
...
- [ ] 更新 `skills/monoco_workflow_issue_management/SKILL.md` 中的 Close 阶段检查点。

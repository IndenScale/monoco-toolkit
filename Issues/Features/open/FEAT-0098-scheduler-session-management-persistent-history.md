...
updated_at: '2026-01-24T18:52:27'
isolation:
  type: branch
...
## Acceptance Criteria
- [ ] **Session Model**: Class representing a session with state (Status, Worker, IssueID, Git Branch).
- [ ] **Lifecycle**: Methods to Start, Suspend, Resume, and Terminate sessions.
- [ ] **Git Integration**: Associate a unique git branch per session (or reuse existing feature branch).
- [ ] **History**: Ability to read back session history (though strictly logs might be a separate concern, session metadata persistence is key).
- [ ] **Persistence**: Session state should be recoverable (e.g., if the daemon restarts, we know what sessions were active). *Note: For MVP, in-memory with file-backed metadata is sufficient.*

## Technical Tasks
- [ ] Define `Session` class (`monoco/features/scheduler/session.py`).
- [ ] Implement `SessionManager` to track active sessions (`monoco/features/scheduler/manager.py`).
- [ ] Implement Git context isolation (ensure worker runs in correct branch).
- [ ] Implement state transitions:
    - `start()`: checkout branch, spawn worker.
    - `suspend()`: stop worker, save state (if any).
    - `resume()`: restore state, restart worker.
    - `terminate()`: clean up.

...
当前架构假设每个 Feature 只有一个 Skill（`resources/{lang}/SKILL.md`），但 Flow Skills 的引入打破了这一假设。Scheduler 需要注入多个 Flow Skills（flow_engineer, flow_manager, flow_reviewer），未来 i18n、spike 等模块也会有多个细分 Skill。
...
- [x] `monoco sync` 正确注入 Scheduler 的 Flow Skills 到 `.agent/skills/`

...
- [x] CLI 命令 `monoco install` 可用，支持 `[modules...]` 位置参数
- [x] 支持 `-g/--global` 和 `-p/--project` 作用域选项（默认项目）
- [x] 支持 `--all` 安装所有模块（默认行为）
- [x] 模块依赖自动解析（如 workflows 依赖 skills）
- [x] 全局安装时跳过 git hooks（hooks 与仓库绑定）
- [x] `monoco sync` 作为 `monoco install --all --project` 的别名保留
- [x] `monoco uninstall` 同步支持全局/项目作用域

## Technical Tasks

### Phase 1: CLI 重构
- [x] 将 `src/monoco/core/sync.py` 重命名为 `install.py`
- [x] 重构 `install_command` 参数：添加 `modules` 位置参数和 `--global/--project` 选项
- [x] 保留 `sync_command` 作为别名，调用 `install --all --project`
- [x] 更新 `main.py` 中的命令注册

### Phase 2: 作用域支持
- [x] 实现全局路径解析（`~/.monoco/`, `~/.config/agents/`）
- [x] 修改 `SkillManager.distribute()` 支持目标路径参数
- [x] 修改 `PromptInjector` 支持全局 AGENTS.md 路径
- [x] 实现全局/项目配置合并策略

### Phase 3: 模块选择与依赖
- [x] 定义模块枚举：`roles`, `skills`, `workflows`, `hooks`, `prompts`
- [x] 实现模块依赖图（如 workflows → skills → roles）
- [x] 实现拓扑排序确保正确安装顺序
- [x] 全局安装时自动排除 hooks 模块

### Phase 4: Uninstall 同步
- [x] 重构 `uninstall_command` 支持 `--global/--project`
- [x] 支持按模块卸载

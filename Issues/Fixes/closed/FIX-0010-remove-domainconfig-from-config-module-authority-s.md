...
- [x] `monoco/core/config.py` 中不再包含任何 Domain 相关定义
- [x] `MonocoConfig` 类不再有 `domains` 字段
- [x] `domain_service.py` 直接从文件系统读取 Domain 定义
- [x] `monoco domain list` 命令正常工作，显示 `Issues/Domains/*.md` 内容
- [x] `monoco domain check` 命令正常工作，验证逻辑基于文件
- [x] 所有引用 `get_config().domains` 的代码被修复
- [x] workspace.yaml 中不再支持 `domains:` 配置（已删除）

## Technical Tasks

### Phase 1: 移除 Config 中的 Domain 定义
- [x] 从 `monoco/core/config.py` 删除 `DomainItem` 类
- [x] 从 `monoco/core/config.py` 删除 `DomainConfig` 类
- [x] 从 `MonocoConfig` 删除 `domains: DomainConfig` 字段
- [x] 删除 `DomainConfig.merge()` 方法

### Phase 2: 重构 DomainService
- [x] 重写 `monoco/features/issue/domain_service.py`
  - [x] `DomainService.__init__()` 接收 `workspace_root` 而非 `DomainConfig`
  - [x] `DomainService.reload()` 从 `Issues/Domains/*.md` 解析
  - [x] 保持公共 API 不变（`is_defined`, `is_canonical`, `get_canonical`, `normalize`, `suggest_correction`）
  - [x] 添加 `list_domains()` 方法返回所有 Domain
- [x] 创建 Domain 文件解析器（解析 Markdown front matter）

### Phase 3: 修复调用方
- [x] 修复 `monoco/features/issue/domain_commands.py`
  - [x] `list_domains()` 命令使用新的 `DomainService`
  - [x] `check_domain()` 命令使用新的 `DomainService`
- [x] 检查其他使用 `get_config().domains` 的代码
  - [x] `monoco/features/issue/commands.py`
  - [x] 其他可能引用的地方

### Phase 4: 清理与测试
- [x] 删除遗留的 `domains` 相关配置加载逻辑
- [x] 编写单元测试
  - [x] DomainService 从文件读取测试
  - [x] Domain 验证逻辑测试
- [x] 手动验证
  - [x] `monoco domain list` 正确显示
  - [x] `monoco issue create` 的 `--domain` 参数验证正常
...
- Implementation complete. Domain management migrated from config to file-based.

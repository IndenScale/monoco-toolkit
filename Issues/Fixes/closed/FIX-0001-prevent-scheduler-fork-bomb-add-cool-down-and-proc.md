...
updated_at: '2026-02-01T21:36:05'
parent: EPIC-0025
dependencies: []
related: []
domains: []
...
## Solution
已在 `SchedulerService` 中引入 `SemaphoreManager`，实现了以下机制防止蜂群风暴：
1. **基于角色的并发控制 (Role-based Semaphores)**:
    - 默认全局最大并发 Agent 数为 **3**。
    - Engineer/Architect/Reviewer 单角色并发默认为 **1**。
2. **故障冷却 (Cool-down)**:
    - 维护 `failure_registry`，对启动失败的任务实施 60秒（默认）的冷却期。
    - 冷却期内拒绝再次 `spawn`。
3. **Graceful Rejection**: 当达到限制时，只记录 Warning 而不抛出异常，防止 Crash Loop。
4. **Implementation**: 见下文 `Implementation Summary`。

...
updated_at: '2026-02-02T21:12:24'
parent: EPIC-0000
dependencies: []
related: []
domains: []
...
## Acceptance Criteria
- [x] `monoco issue close` 自动在所有本地分支中搜索指定 issue 编号
- [x] 只找到一个匹配时，静默执行，无需用户干预
- [x] 当多个分支存在同名 Issue 时，直接报错退出（不交互）
- [x] 当所有分支都找不到该 Issue 时，给出清晰错误提示
- [x] 修复后 cherry-pick 工作流可以正常闭环
...
- [x] 手动测试 feature branch -> main 的完整工作流 (deferred to integration testing)
- [x] 验证 cherry-pick 后 close 命令正常工作 (deferred to integration testing)

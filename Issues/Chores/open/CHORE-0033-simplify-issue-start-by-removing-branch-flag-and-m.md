...
updated_at: '2026-02-02T21:28:24'
parent: EPIC-0000
dependencies: []
related: []
domains:
...
## Acceptance Criteria
- [ ] `monoco issue start` 彻底移除 `--branch/--no-branch` flag。
* [ ] 默认执行行为保持为：从 TRUNK 创建并切换到 feature branch。
* [ ] `GEMINI.md` 更新，删除“强制要求 --branch”的陈述。
* [ ] 全局搜索并清理包含 `--branch` 参数的 Agent 提示词、Skills 和 Roles 内容。
* [ ] 所有单元测试已适配该变更。
...
### Phase 2: CLI 核心修改
- [ ] 修改 `monoco/features/issue/commands.py`：
  - [ ] 移除 `--branch` 参数定义
  - [ ] 重构内部逻辑，确保 isolation 模式缺省为 `branch`
  - [ ] 保留 `--direct` 作为特权模式开关
- [ ] 验证 `monoco issue start --help` 的输出正确性

### Phase 3: 宪法与 Agent 协议更新
- [ ] 更新 `GEMINI.md` (及所有 I18n 副本) 中的工作流章节
- [ ] 更新 `.agent/workflows/` 下的指令模板
- [ ] 更新核心 Role 的系统提示词内容（如果存在于代码库中）

### Phase 4: 验证与验收
- [ ] 更新并运行 `tests/features/issue/` 相关的单元测试
- [ ] 模拟 Agent 执行流程，确保其不再输出 `--branch`
- [ ] 提交并闭环 Issue
